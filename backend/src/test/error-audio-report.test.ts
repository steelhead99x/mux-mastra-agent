...
// Import the TTS tool and related components (commented out unused imports to avoid warnings)
...
describe('Error Audio Report Generation Tests', () => {
  ...
  describe('Error Data Structure Tests', () => {
    ...
    it('should format error percentages correctly', async () => {
      const mockErrors = [
        { count: 35, totalViews: 65, expectedPercentage: 53.85 }, // 35/65 * 100 = 53.846... ≈ 53.85%
        { count: 1, totalViews: 65, expectedPercentage: 1.54 }    // 1/65 * 100 = 1.538... ≈ 1.54%
      ];

      mockErrors.forEach(error => {
        const calculatedPercentage = (error.count / error.totalViews) * 100;
        // Round to 2 decimal places for comparison
        const roundedCalculated = Math.round(calculatedPercentage * 100) / 100;
        expect(roundedCalculated).toBeCloseTo(error.expectedPercentage, 1);
      });
    });
  });
